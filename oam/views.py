...
def index(request, customer_url_id):
    qs_invoices_base = Invoice.objects.order_by("-date_added").annotate(num_of_transactions=Count('transaction'))
    customer = get_object_or_404(
        Customer.objects.prefetch_related( 
                                            ...
                                            Prefetch("invoice_set", 
                                                ...
                                                to_attr="invoices_unpaid"))
        , corp_url_id=customer_url_id)

    return_dict.update({
        'title': 'Invoices',
        'customer_name': customer.corp,
        'invoices_paid': customer.invoices_paid[:5], #invoices.filter(paid=True)[:5],
        'invoices_unpaid': customer.invoices_unpaid[:5], #invoices.filter(paid=False)[:5],
                            ...
                            Invoice.objects
                                ...
                                .prefetch_related(
                                    Prefetch('transaction_set', 
                                        queryset=Transaction.objects.order_by("-date_added"),
                                        to_attr="transactions"),
                                    Prefetch('notes',
                                        queryset=Note.objects.order_by("-date_added"),
                                        to_attr="related_notes"))
                        , id=pk)
    ...
    if(request.POST):
        ...
        return HttpResponseRedirect(
            reverse("oam:invoice_detail",
                    kwargs={'customer_url_id': saved_invoice.customer.corp_url_id,
                            'pk': pk}))
